...
def score_card(
    ...
    v2_stacker, reranker, final = build_recovered_branch(
        images,
        pil_images,
        title,
        description,
...
def build_recovered_branch(
    ...
    if image_encoder is None or multimodal_encoder is None:
        raise RuntimeError("Recovered runtime unavailable: v2 encoders not loaded")
    ...
    try:
        ...
        logger.exception("Recovered runtime failed: %s", exc)
        raise RuntimeError("Recovered runtime failed") from exc
...
def build_prompt_taxonomy_branch(pil_images: list, features: list) -> dict[str, np.ndarray]:
    ...
    if judge is None:
        raise RuntimeError("CLIP prompt judge not loaded")
